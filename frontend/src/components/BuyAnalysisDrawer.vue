<template>
  ...
  >
    ...
    <div class="drawer-body">
      ...
      <el-empty v-else-if="!data" description="暂无买点分析结果" />
      <template v-else>
        ...
        <div class="section-grid">
          ...
          <section class="analysis-section">
            ...
            <div class="data-list">
              ...
              <div class="data-item"><span>买点信号</span><strong>{{ daily?.buy_signal || '-' }}</strong></div>
              <div class="data-item"><span>买点级别</span><strong>{{ daily?.buy_point_level || '-' }}</strong></div>
            </div>
            ...
            <div class="level-list">
              <div v-for="item in daily?.reference_levels || []" :key="item" class="level-row">{{ item }}</div>
            </div>
          </section>
          ...
          <section class="analysis-section">
            ...
            <div class="data-list">
              ...
              <div class="data-item"><span>关键位状态</span><strong>{{ intraday?.key_level_status || '-' }}</strong></div>
            </div>
            <div class="section-emphasis">{{ intraday?.conclusion || '-' }}</div>
          ...
          <section class="analysis-section">
            ...
            <div class="price-grid">
              <div class="metric-card"><span>低吸挂单价</span><strong>{{ orderPlan?.low_absorb_price || '-' }}</strong></div>
              <div class="metric-card"><span>突破挂单价</span><strong>{{ orderPlan?.breakout_price || '-' }}</strong></div>
              <div class="metric-card"><span>回踩确认价</span><strong>{{ orderPlan?.retrace_confirm_price || '-' }}</strong></div>
              <div class="metric-card"><span>放弃价</span><strong>{{ orderPlan?.give_up_price || '-' }}</strong></div>
            </div>
            ...
            <div class="data-list compact-list">
              <div class="data-item"><span>高于哪里不追</span><strong>{{ orderPlan?.above_no_chase || '-' }}</strong></div>
              <div class="data-item"><span>跌破哪里不买</span><strong>{{ orderPlan?.below_no_buy || '-' }}</strong></div>
            </div>
          </section>
...
.overview-card {
  ...
  border-radius: 18px;
  background:
    radial-gradient(circle at top right, rgba(68, 209, 159, 0.16), transparent 36%),
